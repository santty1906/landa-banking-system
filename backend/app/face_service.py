...
# 🔥 NEW: cosine distance (FIX PRINCIPAL)
...
def enroll_faces(username: str, images: dict) -> None:
    ...
    try:
        ...
        for angle in ("frontal", "left", "right"):
            ...
            embeddings[angle] = _get_embedding(temp_path)

        if not _check_embedding_quality(embeddings):
            raise ValueError(
                "Face captures lack sufficient variation. Please try different angles."
            )
...
def verify_face_detailed(username: str, image_base64: str, threshold: float = None) -> dict:
    temp_path = None
    ...
    try:
        ...
        enrolled_avg = payload["averaged"]

        temp_path = _save_temp_image(image_base64)
        probe = _normalize_embedding(_get_embedding(temp_path))

        # 🔥 FIX: cosine comparison instead of L2
        distances = {
            angle: float(_cosine_distance(probe, _normalize_embedding(ref)))
            for angle, ref in enrolled.items()
        }

        avg_distance = float(_cosine_distance(probe, _normalize_embedding(enrolled_avg)))

        best_angle = min(distances, key=distances.get)
        best_distance = min(min(distances.values()), avg_distance)
